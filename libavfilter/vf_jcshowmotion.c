...
static int config_input(AVFilterLink *inlink)
{
    ...
    /* we do not want thicker frame than it make sense */
    if (s->thickness > inlink->w/20/2)
        s->thickness = inlink->w/20/2;
...
{
    ShowMotionContext *s = inlink->dst->priv;
    AVDictionaryEntry *e;
    int ret;
    int i;
    ...
    for (i = 0; i < 15*20; i++) {
        ...
        unsigned char *row[4];
        int drawtop, drawbottom, drawleft, drawright;
        ...
        w = frame->width/20;
        h = frame->height/15;
        drawtop = i < 20 || !s->mip_info[i-20];
        drawbottom = i > 300-20 || !s->mip_info[i+20];
        drawleft = i%20 == 0 || !s->mip_info[i-1];
        drawright = i%20 == 19 || !s->mip_info[i+1];
        for (y = FFMAX(yb, 0); y < frame->height && y < (yb + h); y++) {
            ...
            if (s->invert_color) {
                for (x = FFMAX(xb, 0); x < xb + w && x < frame->width; x++)
                    if (s->thickness >= w/2 ||
                        (y - yb < s->thickness && drawtop) ||
                        (yb + h - 1 - y < s->thickness && drawbottom) ||
                        (x - xb < s->thickness && drawleft) ||
                        (xb + w - 1 - x < s->thickness && drawright))
...
#define OFFSET(x) offsetof(ShowMotionContext, x)
...
AVFilter ff_vf_jcshowmotion = {
    ...
    .priv_size     = sizeof(ShowMotionContext),
